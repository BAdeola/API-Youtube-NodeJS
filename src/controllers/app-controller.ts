...
export const getListVideos = async (request: IncomingMessage, response: ServerResponse) => {
  ...
  response.writeHead(content.statusCode, { 'Content-Type': ContentType.JSON }); //every response should have a header and content
...
export const getFilteredVideos = async (request: IncomingMessage, response: ServerResponse) => {
  ...
  response.writeHead(content.statusCode, { 'Content-Type': ContentType.JSON }); //menas that i'm returning a json
  response.end(JSON.stringify(content.body)); // Return the filtered list of videos as JSON
};